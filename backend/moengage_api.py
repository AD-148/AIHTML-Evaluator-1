        ...
        with response:
            ...
            for line in response.iter_lines():
                if line:
                    ...
                    try:
                        # Extract JSON data
                        # Stream usually sends "data: {...}" or just "{...}"
                        # If it has "data: " prefix strings, strip them.
                        if decoded_line.startswith("data:"):
                            decoded_line = decoded_line[5:].strip()
                        
                        json_data = json.loads(decoded_line)
                        
                        # Navigate Path: preview-payload -> data -> html
                        html_chunk = json_data.get("preview-payload", {}).get("data", {}).get("html")
                        
                        if html_chunk:
                            # Update our buffer with the latest snapshot of the HTML
